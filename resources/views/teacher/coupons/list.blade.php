                    ...
                    <tbody>
                        @forelse ($coupons as $coupon)
                            <tr class="text-center">
                                    ...
                                    <td>{{ $coupon->code }}</td>
                                    <td>{{ $coupon->enabled ? __('Sí') : __('No') }}</td>
                                    <td>{{ $coupon->discount_type }}</td>
                                    <td>{{ $coupon->discount }}</td>
                                    <td>{{ $coupon->created_at->format('d/m/Y') }}</td>
                                    <td>{{ $coupon->expires_at ? $coupon->expires_at->format('d/m/Y') : 'N/A' }}</td>
                                    <td>
                                        <a class="btn btn-outline-dark" href="{{ route('teacher.coupons.edit', ['coupon' => $coupon]) }}">
                        ...
                        <tr class="text-center">
                            <td colspan="7">
                                <div class="empty-results">
                                    {!! __('No tienes ningún cupón todavía: :link', ['link' => "<a href='".route('teacher.coupons.create')."'>Crear mi primer cupón</a>"]) !!}
                                </div>
                            </td>
                        </tr>

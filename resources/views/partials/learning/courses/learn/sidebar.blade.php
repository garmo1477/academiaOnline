<div class="col-xs-12 col-xs-3 col-md-4 col-sm-4 col-lg-3">
    <div class="card">
        ...
        <div class="card-body p-0">
            ...
            @forelse($course->units as $unit)
                {{-- dependiendo del tipo de unidad enseñaremos un contenido u otro
                --}}
                @switch($unit->unit_type)
                    ...
                    <div class="card-text p-2 border-bottom unit">
                        <a 
                            href="#" 
                            id="unit-{{ $index }}" class="text-black-50" 
        ...
        <div class="card-footer">
            @can('review', $course)                
                    <a href="{{ route('courses.reviews.create', ['course' => $course]) }}" class="site-btn btn-block">{{ __('Valorar curso') }}</a>                   
                    
            @endcan
            <a href="{{ route('courses.topics.index', ['course' => $course]) }}" class="site-btn btn-block">{{ __('Ir al foro') }}</a>
        </div>
    </div>
</div>
...
<script>
    ...
    jQuery(document).ready(function () {
        jQuery(".unit").on("click", function (e) {
            ...
            let footerButtons = '';
            //miramos a ver si hay un texto después, si hay añadimos el botón con el siguiente, lo mismo hacemos para el anterior

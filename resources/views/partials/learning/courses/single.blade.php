<div class="mix col-lg-3 col-md-4 col-sm-6 @foreach($course->categories as $category) {{ $category->name }} @endforeach">
    <div class="course-item">
        ...
        {{-- si el usuario está conectado se mostrará los corazones de lista de deseos --}}        
        @auth
            <div class="wish-heart">
                @if ($course->wishedForUser())
                    <i class="fa fa-2x fa-heart-o text-danger toggle-wish" data-route="{{ route('student.wishlist.toggle', ['course' => $course]) }}"></i>
                @else
        ...
        <div class="course-info">
            ...
            <div class="pl-4">
                <a href="{{ route('courses.show', ['course' => $course]) }}" class="site-btn">Información</a>
            </div>
        </div>
    </div>
</div>
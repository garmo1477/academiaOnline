...
trait ManageUnits{
    //todo lo que haga en este trait se beneficiará el TeacherController
    public function units()
    {
        //forTeacher es el scope que definimos en Unit.php
    ...
    {
        //Hacemos esto para luego solo crear un unico formulario que utilizará estos datos
    ...
    public function updateUnit(UnitRequest $request, Unit $unit)
    {
        ...
        if ($request->hasFile('file')) {
            if($unit->file){
                //si ya hay un archivo subido en esa fila de la tabla, lo eliminamos
                Uploader::removeFile('units', $unit->file);
            }
            //y aquí subimos el nuevo archivo
            $file = Uploader::uploadFile('file', 'units');
        }
        //fill sirve para rellenar los datos de un modelo
        $unit->fill($this->unitInput($file))->save(); //usamos save para guardarlo, sin el no funcionará
